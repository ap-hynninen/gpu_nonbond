...
void CudaPMEDirectForce<AT, CT>::calc_14_force(const float4 *xyzq,
  ...
  if (vdw_model_loc == VDW_VSH) {
	...
	if (calc_virial) {
      ...
      std::cout<<"CudaPMEDirectForce<AT, CT>::calc_14_force, Invalid EWALD model "
    ...
    std::cout<<"CudaPMEDirectForce<AT, CT>::calc_14_force, Invalid VDW model"<<std::endl;
    exit(1);
  }
...
void CudaPMEDirectForce<AT, CT>::calc_force(const float4 *xyzq,
  ...
  while (nblock_tot != 0) {
	...
	std::cout<<"CudaPMEDirectForce<AT, CT>::calc_force, Invalid EWALD model "<<elec_model_loc<<std::endl;
	exit(1);
      }
    } else if (vdw_model_loc == VDW_VSW) {
      if (elec_model_loc == EWALD) {
	...
	std::cout<<"CudaPMEDirectForce<AT, CT>::calc_force, Invalid EWALD model "
		 <<elec_model_loc<<std::endl;
	exit(1);
      }
    } else {
      std::cout<<"CudaPMEDirectForce<AT, CT>::calc_force, Invalid VDW model"<<std::endl;
      exit(1);
    }
